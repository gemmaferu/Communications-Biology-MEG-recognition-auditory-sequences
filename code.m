...
for ii = 1:length(v_ID)
    mkdir(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/MRI_nifti_images_revision/00' num2str(v_ID(ii))])
end
...
MRI_path = dir('/projects/MINDLAB2017_MEG-LearningBach/scratch/Leonardo/LearningBach/after_maxfilter_mc/NIFTI_INV2/0*'); %subjects' MRI folders
output_path = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/MRI_nifti_images_revision/0*'); %new subjects' MRI folders
...
for ii = 1:length(MRI_path) %over folders
    niftis = dir([MRI_path(ii).folder '/' MRI_path(ii).name '/*.nii']); %list of nifti files
    for jj = 1:length(niftis) %over nifti files
        if ~strcmp(niftis(jj).name(1), 'y') %not the nifti files that start with 'y'
            file_to_copy = [niftis(jj).folder '/' niftis(jj).name]; %nifti file we need
        end
    end
    new_folders = [output_path(ii).folder '/' output_path(ii).name];
    if file_to_copy(94:95) == new_folders(88:89) %check the subject
        copyfile(file_to_copy,new_folders) %copy the nifti files into the new folders
    end
end
...
if block == 1
    list = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/e80*recogatonal_tsssdsm.mat'); %atonal files
else
    list = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/e80*recogmajor_tsssdsm.mat'); %tonal files
end

%running rhino
%OBS! check that all MEG data are in the same order and number as the MRI nifti files!
mri = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/MRI_nifti_images_revision/00*'); %set path to subjects' MRI folders
for ii = 1:length(list)
    S = [];
    S.ii = ii;
    S.D = [list(ii).folder '/' list(ii).name]; %path to tonal files
    dummymri = dir([mri(ii).folder '/' mri(ii).name '/*.nii']); %path to nifti files
...
block = 2; %1 = atonal; 2 = tonal
freqq = 5; %1 = 0.1-1Hz; 2 = 2-8Hz; 3 = broadband (0.1 - 40 Hz); 4 = 1-4Hz; 5 = 5-8Hz
...
for ii = 1:length(s)
    ...
    elseif freqq == 4
        oat.source_recon.freq_range = [1 4]; %frequency range in Hz
    elseif freqq == 5
        oat.source_recon.freq_range = [5 8]; %frequency range in Hz
    end
...
list_block = dir(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/Block_' num2str(block) '_freq_' num2str(freq) '_*']);
mkdir ('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source', ['FirLev_Block_' num2str(block) '_freq_' num2str(freq) '.oat']);
bs = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_' num2str(freq) '.oat'];
for ii = 1:length(list_block)
    if freqq == 1
        dat = [list_block(ii).folder '/' list_block(ii).name '/concatMfsession' num2str(str2double(list_block(ii).name(25:28))) '_spm_meeg.dat']; %concat file .dat
        mat = [list_block(ii).folder '/' list_block(ii).name '/concatMfsession' num2str(str2double(list_block(ii).name(25:28))) '_spm_meeg.mat']; %concat file .mat
        ses = [list_block(ii).folder '/' list_block(ii).name '/session' num2str(str2double(list_block(ii).name(25:28))) '_recon.mat']; %session file
    elseif freqq == 3
        dat = [list_block(ii).folder '/' list_block(ii).name '/concatMfsession' num2str(str2double(list_block(ii).name(26:29))) '_spm_meeg.dat']; %concat file .dat
        mat = [list_block(ii).folder '/' list_block(ii).name '/concatMfsession' num2str(str2double(list_block(ii).name(26:29))) '_spm_meeg.mat']; %concat file .mat
        ses = [list_block(ii).folder '/' list_block(ii).name '/session' num2str(str2double(list_block(ii).name(26:29))) '_recon.mat']; %session file   
    elseif freqq == 2 || freqq == 4 || freqq == 5
        dat = [list_block(ii).folder '/' list_block(ii).name '/concatMfsession' num2str(str2double(list_block(ii).name(23:26))) '_spm_meeg.dat']; %concat file .dat
        mat = [list_block(ii).folder '/' list_block(ii).name '/concatMfsession' num2str(str2double(list_block(ii).name(23:26))) '_spm_meeg.mat']; %concat file .mat
        ses = [list_block(ii).folder '/' list_block(ii).name '/session' num2str(str2double(list_block(ii).name(23:26))) '_recon.mat']; %session file
    end
    status = movefile(dat,bs);
    status = movefile(mat,bs);
    status = movefile(ses,bs);
    disp(ii)
end

%checking whether all the files were moved
check = dir(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_' num2str(freq) '.oat']);


%% First level, subject level, group level, and creating nifti images

block = 2; %1 = atonal; 2 = tonal
freqq = 5; %1 = 0.1-1Hz; 2 = 2-8Hz; 3 = 0.1-40Hz; 4 = 1-4Hz; 5 = 5-8Hz
...
group_lev = 0; %1 to run group-level analysis
...
fsl_maths_cluster = 1; % 1 to run fsl_maths
...
if block == 1
    ...
    elseif freqq == 4
        outdir = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_1_freq_1_4.oat';
    elseif freqq == 5
        outdir = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_1_freq_5_8.oat';
    end
    list_block = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/e80*recogatonal_tsssdsm.mat');
    s = 1:71;
elseif block == 2
    ...
    elseif freqq == 4
        outdir = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_2_freq_1_4.oat';
    elseif freqq == 5
        outdir = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_2_freq_5_8.oat';
    end
    list_block = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/e80*recogmajor_tsssdsm.mat');
    s = 1:71;
else
...
if ~exist([oat.source_recon.dirname '/' oat.group_level.results_fnames],'var')
    copyfile(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/templates/wholebrain_first_level_minor_group_level_everybody_mask.nii.gz'],[oat.source_recon.dirname '.oat/' oat.first_level.name '_' oat.subject_level.name '_' oat.group_level.name '_mask.nii.gz'])
end
...
if subj_nifti == 1
    ...
    if ~exist([oat.source_recon.dirname '/' oat.group_level.results_fnames],'var')
        copyfile(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/templates/wholebrain_first_level_minor_group_level_everybody_mask.nii.gz'],[oat.source_recon.dirname '/' oat.first_level.name '_mask.nii.gz'])
    end
...
clear DATA
block = 2; %1 = atonal; 2 = tonal
freq = 4; %1 = 0.1-1Hz; 2 = 2-8Hz; 3 = 0.1-40Hz; 4 = 1-4Hz; 5 = 5-8Hz
...
elseif freq == 3
    T = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_0.1_40.oat/wholebrain_first_level_BC_N100_coape_real_subj_group_level_everybody_randomise_c' num2str(contrast) '_dir_tone_' num2str(tone) '/tstat3_gc1_8mm.nii.gz']);
elseif freq == 4
    T = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_1_4.oat/wholebrain_first_level_BC_N100_coape_real_subj_group_level_everybody_randomise_c' num2str(contrast) '_dir_tone_' num2str(tone) '/tstat3_gc1_8mm.nii.gz']);
elseif freq == 5
    T = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_5_8.oat/wholebrain_first_level_BC_N100_coape_real_subj_group_level_everybody_randomise_c' num2str(contrast) '_dir_tone_' num2str(tone) '/tstat3_gc1_8mm.nii.gz']);
end
...
for ii = 1%:2 %over directions of the contrast (cond1>cond2 and cond2>cond1)
    ...
    elseif freq == 3
        S.T = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_0.1_40.oat/wholebrain_first_level_BC_N100_coape_real_subj_group_level_everybody_randomise_c' num2str(contrast) '_dir_tone_' num2str(tone) '/tstat3_gc1_8mm.nii.gz'];
    elseif freq == 4
        S.T = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_1_4.oat/wholebrain_first_level_BC_N100_coape_real_subj_group_level_everybody_randomise_c' num2str(contrast) '_dir_tone_' num2str(tone) '/tstat3_gc1_8mm.nii.gz'];
    elseif freq == 5
        S.T = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_' num2str(block) '_freq_5_8.oat/wholebrain_first_level_BC_N100_coape_real_subj_group_level_everybody_randomise_c' num2str(contrast) '_dir_tone_' num2str(tone) '/tstat3_gc1_8mm.nii.gz'];
    end
    S.outdir = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/4/Block_' num2str(block)]; %output path
...
elseif freq == 3
    if block == 1
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/0.1_40/Block_1';
    else
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/0.1_40/Block_2';
    end
elseif freq == 4
    if block == 1
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/1_4/Block_1';
    else
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/1_4/Block_2';
    end
elseif freq == 5
    if block == 1
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/5_8/Block_1';
    else
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/5_8/Block_2';
    end
end
...
cmd = ['fslmaths ' path '/' name1 ' -add ' path '/' name2 ' -add ' path '/' name3 ' ' path  '/FinalImages/' output]; %OBS! be careful with the spacing
system(cmd)
...
freq = 5; %1 = 0.1-1Hz; 2 = 2-8Hz;3 = 0.1-40Hz; 4 = 1-4Hz; 5 = 5-8Hz
block = 2; %1 = atonal; 2 = tonal; 3 = tonal vs atonal
...
elseif freq == 3
    if block == 1
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/0.1_40/Block_1/FinalImages';
    elseif block == 2
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/0.1_40/Block_2/FinalImages';
    else
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastMajorAtonal/Freq_2_8/Contrast1/Cluster_Permutation_test/FinalImages';
    end
elseif freq == 4
    if block == 1
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/1_4/Block_1/FinalImages';
    elseif block == 2
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/1_4/Block_2/FinalImages';
    end
elseif freq == 5
    if block == 1
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/5_8/Block_1/FinalImages';
    elseif block == 2
        path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/OldvsNew_MCS/5_8/Block_2/FinalImages';
    end
end
...
%% COMPARING SLOW (0.1 - 1 Hz) VS. FAST (2 - 8 Hz) FREQUENCY BANDS

%here we follow these steps:
%(1) t-tests
%(2) cluster permutations test
%(3) combining images with significant clusters
%(4) reporting the information on Excel tables

%% T-tests

subj_slow = zeros(23,27,23,526,71); %5D matrix with voxel coordinates and timepoints for all subjects  
subj_fast = zeros(23,27,23,526,71); %5D matrix with voxel coordinates and timepoints for all subjects 

block = 1; %1 = atonal; 2 = tonal
contrast = 3; %set contrast to 1 2 or 3

for ii = 1:71 %over subjects
    if block == 1
        dum2 = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_1_freq_0.1_1.oat/subject' num2str(ii) '_wholebrain_first_level_BC_coape_subj_dir/tstat' num2str(contrast) '_8mm.nii.gz']);
        subj_slow(:,:,:,:,ii) = dum2.img; %4D matrix for each subject
        dum2 = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_1_freq_2_8.oat/subject' num2str(ii) '_wholebrain_first_level_BC_N100_coape_real_subj_dir/tstat' num2str(contrast) '_8mm.nii.gz']);
        subj_fast(:,:,:,:,ii) = dum2.img; %4D matrix for each subject
    else
        dum2 = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_2_freq_0.1_1.oat/subject' num2str(ii) '_wholebrain_first_level_BC_coape_subj_dir/tstat' num2str(contrast) '_8mm.nii.gz']);
        subj_slow(:,:,:,:,ii) = dum2.img;
        dum2 = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_2_freq_2_8.oat/subject' num2str(ii) '_wholebrain_first_level_BC_N100_coape_real_subj_dir/tstat' num2str(contrast) '_8mm.nii.gz']);
        subj_fast(:,:,:,:,ii) = dum2.img;
    end
    disp(ii)
end
...
for jj = 1:length(onset) %over onset of tones
    ...
    time_window_slow = subj_slow(:,:,:,onset(jj):onset(jj) + x,:); %time window of tone x for major
    dpm1 = squeeze(mean(time_window_slow,4)); %average of tone x for major
    %the fourth dimension (time) is eliminated using 'squeeze' since it corresponds to only one point in time
    time_window_fast = subj_fast(:,:,:,onset(jj):onset(jj) + x,:); %time window of tone x for atonal
    dpm2 = squeeze(mean(time_window_fast,4)); %average of tone x for atonal
    ...
    pathfreq = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block' num2str(block) '/Contrast' num2str(contrast)];
    if ~exist(pathfreq,'dir')
        mkdir(pathfreq) %create a new folder for each contrast
    end
    ...
    %use dum2 to index a file with only four dimensions (3 voxel dimensions and 1 subject dimension)
    dum2.img = T; %write the image
    save_nii(dum2,[pathfreq '/Tval_SlowvsFast_tone_' num2str(jj) '.nii.gz']); %save the nifti image for the t-values
    dum2.img = P; %write the image
    save_nii(dum2,[pathfreq '/Pval_SlowvsFast_tone_' num2str(jj) '.nii.gz']); %save the nifti image for the p-values
    disp(jj)
end
...
block = 2; %1 = atonal; 2 = tonal
contrast = 2; %set to 1 2 or 3
tone = 2; %set to 1 2 3 4 or 5

if block == 1
    P = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block1/Contrast' num2str(contrast) '/Pval_SlowvsFast_tone_' num2str(tone) '.nii.gz']);
    T = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block1/Contrast' num2str(contrast) '/Tval_SlowvsFast_tone_' num2str(tone) '.nii.gz']);
else
    P = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block2/Contrast' num2str(contrast) '/Pval_SlowvsFast_tone_' num2str(tone) '.nii.gz']);
    T = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block2/Contrast' num2str(contrast) '/Tval_SlowvsFast_tone_' num2str(tone) '.nii.gz']);
end
...
data(data==0) = NaN; %removing non-brain voxels
P2(P2==0) = NaN; %removing non-brain voxels
data(P2<0.99999) = NaN; %removing non-significant voxels
...
for ii = 1:2 %over directions of the contrast (cond1>cond2 and cond2>cond1)
    S = [];
    if block == 1
        S.T = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block1/Contrast' num2str(contrast) '/Tval_SlowvsFast_tone_' num2str(tone) '.nii.gz']; %path to image with t-values
        S.outdir = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block1/Contrast' num2str(contrast) '/Cluster_Permutation_test']; %output path
    else
        S.T = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block2/Contrast' num2str(contrast) '/Tval_SlowvsFast_tone_' num2str(tone) '.nii.gz']; %path to image with t-values
        S.outdir = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block2/Contrast' num2str(contrast) '/Cluster_Permutation_test']; %output path
    end
    S.parcelfile = '/projects/MINDLAB2017_MEG-LearningBach/scripts/Leonardo_FunctionsPhD/External/aal_8mm_try5.nii.gz';
    S.labels = '/projects/MINDLAB2017_MEG-LearningBach/scripts/Leonardo_FunctionsPhD/External/AAL_labels.mat';
    S.MNIcoords = mni_coords; %MNI coordinates of 8mm MNI152T1 brain
    ...
    if ii == 1
        S.anal_name = ['Tval_SlowvsFast_tone_' num2str(tone) '_Cond1vsCond2']; %name for the analysis (used to identify and save image and results)
    else
        S.anal_name = ['Tval_SlowvsFast_tone_' num2str(tone) '_Cond2vsCond1']; %name for the analysis (used to identify and save image and results)
    end
...
block = 2; %1 = 0.1-1Hz; 2 = 2-8Hz
contrast = 2; %set to 1 2 or 3
tone = 1; %set to 1 2 3 4 or 5

if block == 1
    %path to cluster permutation test images
    path = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block' num2str(block) '1/Contrast' num2str(contrast) '/Cluster_Permutation_test'];
else
    path = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block' num2str(block) '/Contrast' num2str(contrast) '/Cluster_Permutation_test'];
end

%cond1>con2
name1 = ['Tval_SlowvsFast_tone_' num2str(tone) '_Cond1vsCond2_SignClust_1_Tvals.nii.gz'];

%cond2>cond1
name2 = ['Tval_SlowvsFast_tone_' num2str(tone) '_Cond2vsCond1_SignClust_1_Tvals.nii.gz'];


output = ['Tval_SlowvsFast_tone_' num2str(tone) '_All_Clusters.nii.gz'];
...
block = 2; %1 = atonal; 2 = tonal
contrast = 2; %1 = memorized; 2 = novel

path = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/Block' num2str(block) '/Contrast' num2str(contrast) '/Cluster_Permutation_test/FinalImages'];


for tonebytone = 1:5
    %tone x cluster 1
    fname = [path '/Tval_SlowvsFast_tone_' num2str(tonebytone) '_All_Clusters.nii.gz'];
    %getting MNI coordinates of significant voxels within the provided image
    [ mni_coords, xform ] = osl_mnimask2mnicoords(fname);
    %loading the image
    V = nii.load(fname);
    ...
    PDn = cell2table(PD(~any(cellfun('isempty',PD),2),:)); %remove the possible empty cell
    writetable(PDn,['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFrequency/SupplementaryTablesSourceClusters/Block_' num2str(block) '_contrast_' num2str(contrast) '.xlsx'],'Sheet',tonebytone) %printing excel file
end

%% CORRELATION WITH FAMILIARITY SCORES

%here we follow these steps:
%(1) correlation with familiarity scores
...
list_familiarity = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/behavioral_data/familiarity_LB2017.xlsx'); %dir to working memory excel file
[~,~,familiarity] = xlsread([list_familiarity(1).folder '/' list_familiarity(1).name]); %loads working memory data
...
for jj = 1:length(onset) %over onset of tones 
    ...
    fam = cell2mat(familiarity(2:end,3)); %familiarity scores
    idxnonans = ~isnan(fam); %1 for numbers, 0 for non-values
    for pp = 1:sz(1) %first dimension of voxel coordinates
        for yy = 1:sz(2) %second dimension of voxel coordinates
            for zz = 1:sz(3) %third dimension of voxel coordinates
                if dpm1(pp,yy,zz,1) ~= 0 %only for significant brain values
                    a = squeeze(dpm1(pp,yy,zz,idxnonans));
                    [rho,p] = corr(a,fam(idxnonans)); %correlation for tone x
    ...
    %storing the images
    pathcontrast = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFamiliarity';
...
T = load_nii(['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFamiliarity/Rho_WM_tone_' num2str(tone) '.nii.gz']);
...
for ii = 1 %over directions of the contrast (cond1>cond2 and cond2>cond1)
    S = []; %empty structure
    S.T = ['/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFamiliarity/Rho_WM_tone_' num2str(tone) '.nii.gz'];
    S.outdir = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFamiliarity/MCS'; %output path
    S.parcelfile = '/projects/MINDLAB2017_MEG-LearningBach/scripts/Leonardo_FunctionsPhD/External/aal_8mm_try5.nii.gz';
    S.labels = '/projects/MINDLAB2017_MEG-LearningBach/scripts/Leonardo_FunctionsPhD/External/AAL_labels.mat';
    S.MNIcoords = mni_coords; %MNI coordinates of 8mm MNI152T1 brain
    S.data = DATA{ii};
...
path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFamiliarity/MCS';
...
path = '/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFamiliarity/MCS/FinalImages';
...
for tonebytone = 1:5
    ...
    PDn = cell2table(PD(~any(cellfun('isempty',PD),2),:)); %remove the possible empty cell
    writetable(PDn,'/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/ContrastFamiliarity/Correlation_Familiarity_0.1_1.xlsx','Sheet',tonebytone); %printing excel file
end


%% Extracting key data for sharing purposes

%the neuroimaging data is available on Zenodo:

analys_l = 4; %1 = 0.1-1Hz tonal;2 = 0.1-1Hz atonal; 3 = 2-8Hz tonal;4 = 2-8Hz atonal

if analys_l == 1
    list = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_2_freq_0.1_1.oat/subj*.mat');
    outdir = '/aux/MINDLAB2021_MEG-TempSeqAges/Upload_Donwload/Gemma_CB/MEG_sourcedata/Tonal_01_1Hz';
    mkdir(outdir)
elseif analys_l == 2
    list = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_1_freq_0.1_1.oat/subj*.mat');
    outdir = '/aux/MINDLAB2021_MEG-TempSeqAges/Upload_Donwload/Gemma_CB/MEG_sourcedata/Atonal_01_1Hz';
    mkdir(outdir)
elseif analys_l == 3
    list = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_2_freq_2_8.oat/subj*.mat');
    outdir = '/aux/MINDLAB2021_MEG-TempSeqAges/Upload_Donwload/Gemma_CB/MEG_sourcedata/Tonal_2_8Hz';
    mkdir(outdir)
elseif analys_l == 4
    list = dir('/scratch7/MINDLAB2020_MEG-AuditoryPatternRecognition/gemma/source/FirLev_Block_1_freq_2_8.oat/subj*.mat');
    outdir = '/aux/MINDLAB2021_MEG-TempSeqAges/Upload_Donwload/Gemma_CB/MEG_sourcedata/Atonal_2_8Hz';
    mkdir(outdir)
end

for ii = 1:length(list)
   load([list(ii).folder '/' list(ii).name]) 
   data = oat_stage_results.cope;
   save([outdir '/SUBJ' num2str(ii)],'data')
   disp(ii)
end
